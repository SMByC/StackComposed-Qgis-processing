...
EXTLIBS_DOWNLOAD_URL = f"{PLUGIN_REPOSITORY}/releases/download/{_get_plugin_version()}/extlibs.zip"


def _log(msg: str, level: str = "Info") -> None:
    try:
        qgis_level = getattr(getattr(Qgis, "MessageLevel", Qgis), level)
        QgsMessageLog.logMessage(msg, tag=PLUGIN_NAME, level=qgis_level)
    except Exception:
...
class DownloadAndUnzip(QDialog):
    def __init__(self, url: str, output_path: str, parent=None):
        ...
        self.setWindowTitle("StackComposed Installation")
        self.setModal(True)
        ...
        if extracted_ok:
            ...
            QMessageBox.critical(
                None,
                "StackComposed: Error installing libs",
                (
                    "Error downloading and extracting additional Python packages"
                    " required for StackComposed.\n\n"
                    "Read the install instructions here:\n"
                    f"{PLUGIN_REPOSITORY}#installation"
                ),
